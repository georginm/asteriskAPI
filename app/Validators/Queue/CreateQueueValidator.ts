...
class CreateQueueValidator {
  constructor(protected ctx: HttpContextContract) {}

  public schema = schema.create({
    name: schema.string({ trim: true }, [rules.maxLength(128)]),
    context: schema.string({ trim: true }, [rules.maxLength(128)]),
    joinempty: schema.string({ trim: true }, [rules.maxLength(128)]),
    leavewhenempty: schema.string({ trim: true }, [rules.maxLength(128)]),
    monitorType: schema.string({ trim: true }, [rules.maxLength(128)]),
    musiconhold: schema.string({ trim: true }, [rules.maxLength(128)]),

    timeout: schema.number(),
    wrapuptime: schema.number(),

    ringinuse: schema.enum(['yes', 'no'] as const),

    strategy: schema.enum([
      'ringall',
      'leastrecent',
      'fewestcalls',
      'random',
