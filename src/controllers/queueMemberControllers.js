...
module.exports = {
    ...
    async update(req, res){
        try {
            var query = await db.select('queue_members', "interface", {interface: req.body.interface});
            // Essa interface já está em algum registro do banco?
            if(query.length == 0){ // Caso não tenha registro
                return res.status(401).json({error: "Não há registro dessa interface no banco de dados"});
            }
            else{ // Caso tenha
                query = await db.update('queue_members',req.body, {interface: req.body.interface});
